	...
	LPARAM longParam)
{
	...
	{
	case WM_SIZE: {
		if (gui::device && wideParam != SIZE_MINIMIZED)
		{
			gui::presentParameters.BackBufferWidth = LOWORD(longParam);
			gui::presentParameters.BackBufferHeight = HIWORD(longParam);
			gui::ResetDevice();
		}
	} return 0;
	...
	case WM_LBUTTONDOWN: {
		gui::position = MAKEPOINTS(longParam); // sets click points
	}return 0;
		
	case WM_MOUSEMOVE: {
		if (wideParam == MK_LBUTTON) // if left mouse button is held down
		{
